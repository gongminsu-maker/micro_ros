...
// IMU Offset mode 선택
String offsetMode = "dynamic"; //""manual"; // 기본 모드 manual로 설정

// 수동 offset 값 입력부
// 현재 Red light imu에 맞춰둠
float offsetXAccel = -0.787833649;
float offsetYAccel = -0.242523277;
float offsetZAccel = 10.878856883;
...
void timer_callback_imu(rcl_timer_t * timer_imu, int64_t last_call_time){
  RCLC_UNUSED(last_call_time);
  if (timer_imu != NULL){
...
void setup() {
  ...
  //imu 초기화 및 연결 확인코드
    ...
    RCCHECK(rclc_timer_init_default(&timer_imu, &support, RCL_MS_TO_NS(10), timer_callback_imu));
